...
/**
 * Field definition used when creating an object kind through the admin UI.
 */
interface ObjectKindFieldData {
  name: string;
  type?: string;
  required?: boolean;
  public?: boolean;
...
/**
 * Object kind configuration for createObjectKind().
 */
interface ObjectKindData {
  label: string;
  labelPlural?: string;
  description?: string;
  slug?: string;
  categorized?: boolean;
  fields?: ObjectKindFieldData[];
}

/**
 * Data for creating a museum object or page.
 */
interface PostData {
  title: string;
  content?: string;
 ...
 * @throws {Error} If plugin cannot be found or activation fails
 */
async function activateMuseumPlugin(page: Page): Promise<boolean> {
 ...
 */
async function setupMuseumTest(
  page: Page,
  adminUser: string | null = null,
  adminPass: string | null = null,
): Promise<void> {
 ...
 */
async function deleteAllObjectKinds(page: Page): Promise<void> {
 ...
 */
async function deleteAllMuseumPosts(page: Page): Promise<void> {
  await page.goto("/wp-admin/");
  await page.waitForLoadState("domcontentloaded");
  const nonce = await page.evaluate(
    () => (window as any).wpApiSettings?.nonce as string | undefined,
  );

  const resp = await page.request.get("/wp-json/wp-museum/v1/mobject_kinds", {
    headers: { "X-WP-Nonce": nonce as string },
  });
  if (!resp.ok()) return;
 ...
 */
async function deleteAllObjectsOfType(
  page: Page,
  postType: string,
): Promise<void> {
  await page.goto("/wp-admin/");
 ...
 * });
 */
async function createObjectKind(
  page: Page,
  kindData: ObjectKindData,
): Promise<void> {
  ...
  if (kindData.fields && kindData.fields.length > 0) {
    for (const field of kindData.fields) {
      ...
      let fieldNameInput: Locator | null = null;
      ...
      let typeSelect: Locator | null = null;
 ...
 */
async function createSimpleObjectKind(
  page: Page,
...
function postTypeFromSlug(slug: string): string {
 ...
 * @returns {Promise<void>}
 */
async function createMuseumObject(
  page: Page,
  postType: string,
  objectData: PostData,
): Promise<void> {
  ...
  if (isBlockEditor) {
    ...
    let contentArea: Locator | null = null;
    ...
    if (objectData.fields) {
      for (const [fieldName, fieldValue] of Object.entries(objectData.fields)) {
        ...
        let fieldInput: Locator | null = null;
 ...
 * @returns {Promise<void>}
 */
async function dismissEditorModals(page: Page): Promise<void> {
  ...
  if (modalExists) {
    ...
    for (const selector of closeSelectors) {
      try {
        ...
        console.log(`Close button ${selector} failed: ${(error as Error).message}`);
      }
    }
 ...
 * @returns {Promise<void>}
 */
async function insertMuseumBlock(
  page: Page,
  blockName: string,
  ...
  let inserterButton: string | null = null;
  ...
  let blockButton: Locator | null = null;
 ...
 * @returns {Promise<string>} URL of the created page
 */
async function createPageWithBlock(
  page: Page,
  title: string,
  blockName: string,
  blockSelector: string,
