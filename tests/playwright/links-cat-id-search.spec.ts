...
import type { ObjectKind, ObjectFieldsResponse } from "../../src/types";

test.describe("Cat ID search via /wp/v2/search (#115)", () => {
  let objectPostType: string;
  let catFieldSlug: string;
  let objectPostId: number;
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    const fields: ObjectFieldsResponse = await fieldsResp.json();
