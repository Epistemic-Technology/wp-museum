...
import { loginAsAdmin, setupMuseumTest, createSimpleObjectKind } from "./utils";
import type {
  ObjectKind,
  MObjectField,
  FieldLinkValue,
...
test.describe("Links frontend: cat IDs and custom labels (#115)", () => {
  let objectPostType: string;
  let linksFieldSlug: string;
  let internalTargetPostId: number;
  let internalTargetTitle: string;
  let internalTargetCatId: string;
  let hostObjectPostId: number;
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    const fields: Record<string, MObjectField> = await fieldsResp.json();
    ...
    const linksValue: FieldLinkValue[] = [
      {
        type: "post",
