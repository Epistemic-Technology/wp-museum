...
import { loginAsAdmin, deleteAllObjectKinds } from "./utils";
import fs from "fs";
import path from "path";

/**
 * Shape of a field in the instrument kind-export JSON fixture
 * (tests/data/museum/instrument-simplified.json). Not covered by
 * src/types — the export format is looser than the wire MObjectField.
 */
interface KindExportField {
  slug: string;
  name: string;
  type: string;
  public: boolean;
...
/**
 * Shape of the kind-export JSON (fixture and exported download).
 */
interface KindExportSchema {
  label: string;
  label_plural: string;
  description: string;
  categorized?: boolean;
  hierarchical?: boolean;
  fields: Record<string, KindExportField>;
}
 ...
 */
async function handleSave(page: Page): Promise<void> {
 ...
 */
async function countObjectKinds(page: Page): Promise<number> {
...
test.describe("Museum Object Kinds", () => {
  ...
  // TODO(ts-migration): Playwright's typed overloads are test(title, body)
  // or test(title, details, body); this pre-existing call passes the body
  // second and the details object third, so the { timeout } argument is
  // ignored at runtime. Cast to keep the current behavior unchanged.
  (test as any)(
    ...
    async ({ page }: { page: Page }) => {
      ...
      const expectedSchema: KindExportSchema = JSON.parse(
      ...
      const exportedSchema: KindExportSchema = JSON.parse(exportedContent);
