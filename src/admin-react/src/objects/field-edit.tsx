...
/**
 * Updates a single property of a field. `value` is a string from text/select
 * inputs or a boolean from checkboxes.
 */
...
interface DimensionFieldsProps {
  /** Dimensions data for this measure field. */
  dimensionsData: FieldDimensions;
  /** Database id of the field being edited. */
  fieldId: number;
  /** Updates a single property of the field. */
...
interface FieldTypeControlsProps {
  /** Database id of the field being edited. */
  fieldId: number;
  /** Field type; may be `""` for an unconfigured field. */
  type: MObjectFieldType | "";
  /** Maximum characters allowed (plain/rich fields; 0 = no limit). */
  maxLength: number;
  /** Factor options (factor/multiple fields). */
  factors: string[];
  /** Unit of measurement (measure fields). */
  units: string;
  /** Dimensions data for measure fields. */
  dimensionsData: FieldDimensions;
  /** Updates a single property of the field. */
  updateField: UpdateFieldFunction;
  /** Replaces the field's factors with a new list. */
  updateFactors: (fieldId: number, newFactors: string[]) => void;
  /** Whether this field is unsaved (negative field_id). */
  newField: boolean;
}
...
interface FieldEditProps {
  /** The field definition being edited. */
  fieldData: MObjectField;
  /** Updates a single property of the field. */
  updateField: UpdateFieldFunction;
  /** Replaces the field's factors with a new list. */
  updateFactors: (fieldId: number, newFactors: string[]) => void;
  /** Deletes the field (after confirmation). */
  deleteField: (fieldId: number) => void;
  /** Default dimensions data used when the field has none. */
  dimensionsDefault: FieldDimensions;
  /** Props spread onto the drag handle for drag-and-drop reordering. */
  dragHandleProps: HTMLAttributes<HTMLDivElement>;
}

const FieldEdit = (props: FieldEditProps) => {
  const {
    fieldData,
    updateField,
    updateFactors,
  ...
  const getFieldTypeLabel = (type: MObjectFieldType | "") => {
