 ...
 *
 * All four arguments are required: the implementations record the refresh
 * time and hand results back through the last two.
 */
...
/**
 * Search text shorter than this is not worth a round trip.
 *
 * @type {number}
 */
...
const SearchBox = (props: SearchBoxProps) => {
  ...
  /**
   * Runs a search, supplying the callbacks the fetch implementations need to
   * report back through. Every search path goes through here so that none of
   * them can leave one out.
   *
   * @param {string}  text      The text to search for, if there is any.
   * @param {boolean} titleOnly Whether to search just the post title.
   */
  const runSearch = (text: string | null, titleOnly: boolean) => {
    if (!text || text.length < minSearchTextLength) {
      return;
    }
    fetchSearchResults(text, titleOnly, setLastRefresh, setResults);
  };
  ...
  const onChangeSearchText = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const sinceLastRefresh = Date.now() - lastRefresh.getTime();
    if (sinceLastRefresh > refreshInterval) {
      runSearch(content, onlyTitle);
    } else {
      // Too soon since the last request: wait out the rest of the interval.
      setTimerId(
        setTimeout(
          () => runSearch(content, onlyTitle),
  ...
  const onTitleToggle = () => {
    ...
    setSelectedItem(0);
    runSearch(searchText, !onlyTitle);
  };
